...
def log_in(request):
    ...
    if auth['logged_in']:
        return render(request, 'login.html', {'message':{'status_message':"You're already logged in as " + auth['username'] + "!"}, 'logged_in':auth['username']})
    if request.method == "POST":
